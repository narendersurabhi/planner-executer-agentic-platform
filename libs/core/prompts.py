...
def resume_tailoring_prompt(job: dict[str, Any]) -> str:
    ...
    return (
        ...
        "- summary: string (3 to 4 lines)\n"
        "- skills: array of {term: string, definition: string} ordered by relevance\n"
        "- experience: array of {company, title, location, dates, bullets: string[]}\n"
        "- education: array of {degree, school, location, dates}\n"
        "- certifications: array of strings\n"
        "- credibility_gate: array of strings (rewritten bullets that needed defensibility)\n"
        "- one_page_selection: array of strings (best 6 to 8 bullets for most recent role)\n\n"
        "Rules for header and experience:\n"
        "- Extract header fields from the candidate resume. Do not invent them.\n"
        ...
        "If any required header or experience fields are missing in the candidate resume,\n"
        "return ONLY this JSON object and nothing else:\n"
        "{\n"
        "  \"error\": \"missing_required_fields\",\n"
        "  \"missing_fields\": [\"header.name\", \"experience[0].company\"]\n"
        "}\n\n"
        "Rules for education and certifications:\n"
