...
def _docx_generate_from_spec(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {"path": str(output_path), "bytes_written": bytes_written}
...
def _safe_join_artifacts_path(path: str) -> Path:
    ...
    if not str(candidate).startswith(str(artifacts_dir.resolve())):
...
def _resolve_items(value: Any, context: Dict[str, Any], strict: bool) -> List[Any]:
    ...
    if isinstance(value, str):
        ...
        if isinstance(resolved, list):
            return resolved
        if strict:
            _tool_error("bullets items must resolve to an array")
        return [resolved]
    if value is None:
        return []
    if strict:
        _tool_error("bullets items must be an array")
    return [value]
    ...
    for token in tokens:
        if isinstance(token, int):
            if isinstance(current, list) and 0 <= token < len(current):
                ...
                continue
            _tool_error(f"Index out of range: {token}")
